...
### 2) Fine-tuning on sequence tagging task:
Example fine-tuning 
```
...
fairseq-train data/kb_ner/bin \
    --user-dir src --task sequence_tagging --arch transformer_pretrain_tagging_base \
...
### 3) Inference for MEPAVE test data using above trained checkpoint.

```
cd fairseq_cli_ext/

export CUDA_VISIBLE_DEVICES=2
DATA_DIR=../data/kb_ner/bin
MODEL=../models/finetune/ner/checkpoint.pt
HEAD=transformer_tagging_head
